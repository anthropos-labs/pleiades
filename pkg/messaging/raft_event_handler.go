...
func (r *RaftEventHandler) WaitForLeaderUpdate(shardId uint64, timeout time.Duration) <-chan *raftv1.RaftEvent {
	...
	for event := range listener {
		...
		if payload.GetAction() == raftv1.Event_EVENT_LEADER_UPDATED {
			...
			if nodeEvent.GetShardId() == shardId {
				err := event.AckSync()
				if err != nil {
					r.logger.Error().Err(err).Str("subject", RaftSubject).Msg("error acknowledging")
				}
				results <- payload
			}
		}
