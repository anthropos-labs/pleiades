...
import (
	...
	"a13s.io/pleiades/pkg/fsm/systemstore"
	"a13s.io/pleiades/pkg/messaging/workflow/runtime/graph"
...
type PartialState struct {
	uprequest *ExecutorRequest
}

func DecodePartialRequest(encodedState []byte) (*PartialState, error) {
...
func NewFlowExecutor(logger zerolog.Logger, stateStore systemstore.StateStore, dataStore systemstore.DataStore, executor IExecutor) *FlowExecutor {
	return &FlowExecutor{logger: logger, stateStore: stateStore, dataStore: dataStore, executor: executor}
}

// FlowExecutor faas-flow executor
type FlowExecutor struct {
	flow *graph.Pipeline // the faas-flow
	...
	logger          zerolog.Logger         // Handle flow logs
	stateStore      systemstore.StateStore // the state store
	dataStore       systemstore.DataStore  // the data store
...
func (fexec *FlowExecutor) incrementCounter(counter string, incrementBy int) (int, error) {
	...
	for i := 0; i < counterUpdateRetryCount; i++ {
		...
		if err != nil {
			...
			if err != nil {
				serr = errors.Newf("failed to update counter %s, error %v", counter, err)
				continue
			}
		...
		if err != nil {
			return 0, errors.Newf("failed to update counter %s, error %v", counter, err)
		}
	...
	return 0, errors.Newf("failed to update counter after max retry for %s, error %v", counter, serr)
}
...
func (fexec *FlowExecutor) retrieveCounter(counter string) (int, error) {
	encoded, err := fexec.stateStore.Get(counter)
	if err != nil {
		return 0, errors.Newf("failed to get counter %s, error %v", counter, err)
	}
	current, err := strconv.Atoi(encoded)
	if err != nil {
		return 0, errors.Newf("failed to get counter %s, error %v", counter, err)
...
func (fexec *FlowExecutor) storePartialState(partialState *PartialState) error {
	...
	for i := 0; i < counterUpdateRetryCount; i++ {
		...
		if err != nil {
			...
			if err != nil {
				serr = errors.Newf("failed to update partial-state, error %v", err)
				continue
			}
			return nil
		}
		...
		if err != nil {
			return errors.Newf("failed to update partial-state, error %v", err)
		}
	...
	return errors.Newf("failed to update partial-state after max retry, error %v", serr)
}
...
func (fexec *FlowExecutor) retrievePartialStates() ([]*PartialState, error) {
	...
	encoded, err := fexec.stateStore.Get(key)
	if err != nil {
		return nil, errors.Newf("failed to retrive partial-state, error %v", err)
	}
	...
	for _, state := range encodedStates {
		ps, err := DecodePartialRequest([]byte(state))
		if err != nil {
			return nil, errors.Newf("failed to retrive partial-state, error %v", err)
		}
...
func (fexec *FlowExecutor) isActive() bool {
	...
	return state == StateRunning || state == StatePaused
}
...
func (fexec *FlowExecutor) executeNode(request []byte) ([]byte, error) {
	...
	for _, operation := range currentNode.Operations() {
		...
		if err != nil {
			err = errors.Newf("Node(%s), IOperation (%s), error: execution failed, %v",
...
func (fexec *FlowExecutor) findCurrentNodeToExecute() {
	...
	for true {
		...
		fexec.flow.UpdatePipelineExecutionPosition(graph.DepthIncrement, currentNode.Id)
	}
}
...
func (fexec *FlowExecutor) forwardState(currentNodeId string, nextNodeId string, result []byte) error {
	...
	defaultStore, ok := fexec.dataStore.(*systemstore.InMemoryDataStore)
	if ok {
	...
	if fexec.isPaused() {
		...
		fexec.log("[request `%s`] ExecutorRequest is paused, storing partial state for node: %s\n", fexec.id, nextNodeId)
...
func (fexec *FlowExecutor) executeDynamic(context *Context, result []byte) ([]byte, error) {
	// get pipeline
	pipeline := fexec.flow
	...
	subdags := make(map[string]*graph.Dag)
	options := []string{}
	...
	if branchCount == 0 {
		return nil, errors.Newf("[request `%s`] Dynamic Node %s, failed to execute as condition/foreach returned no option",
			fexec.id, currentNodeUniqueId)
	}
	...
	if err != nil {
		return nil, errors.Newf("[request `%s`] Failed to initiate dynamic in-degree count for %s, err %v",
			fexec.id, key, err)
	}
	...
	if err != nil {
		return nil, errors.Newf("[request `%s`] Dynamic Node %s, failed to store dynamic options",
			fexec.id, currentNodeUniqueId)
	}
	...
	for option, subdag := range subdags {
		...
		if currentNode.GetForwarder("dynamic") != nil {
			...
			if serr != nil {
				return []byte(""), errors.Newf("failed to store intermediate result, error %v", serr)
			}
		...
		pipeline.UpdatePipelineExecutionPosition(graph.DepthIncrement, subNode.Id)
		...
		if forwardErr != nil {
			...
			pipeline.UpdatePipelineExecutionPosition(graph.DepthDecrement, currentNode.Id)
			return nil, errors.Newf("Node(%s): error: %v",
...
func (fexec *FlowExecutor) handleDynamicEnd(context *Context, result []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Newf("failed to retrive dynamic options for %v, error %v",
	...
	if len(options) > 1 {
		...
		if currentNode.GetForwarder("dynamic") != nil {
			...
			if err != nil {
				return nil, errors.Newf("failed to store branch result of dynamic node %s for option %s, error %v",
		...
		if err != nil {
			return []byte(""), errors.Newf("failed to update inDegree counter for node %s", currentNode.GetUniqueId())
		}
	...
	if serr != nil {
		serr := errors.Newf("failed to aggregate dynamic node data, error %v", serr)
		return nil, serr
	}
	if data == nil {
		data = []byte("")
...
func (fexec *FlowExecutor) handleNextNodes(context *Context, result []byte) ([]byte, error) {
	// get pipeline
	pipeline := fexec.flow
	...
	for _, node := range nextNodes {
		...
		if inDegree > 1 {
			...
			if err != nil {
				return []byte(""), errors.Newf("failed to update inDegree counter for node %s", node.GetUniqueId())
			}
		...
		pipeline.UpdatePipelineExecutionPosition(graph.DepthSame, node.Id)
		...
		if forwardErr != nil {
			// reset dag execution position
			pipeline.UpdatePipelineExecutionPosition(graph.DepthSame, currentNode.Id)
			return nil, errors.Newf("Node(%s): error: %v",
...
func (fexec *FlowExecutor) handleFailure(context *Context, err error) {
	var data []byte

	context.State = StateFailure
	...
	if fexec.flow.Finally != nil {
		fexec.log("[request `%s`] calling Finally handler with state: %s\n",
			fexec.id, StateFailure)
		fexec.flow.Finally(StateFailure)
	}
	if data != nil {
...
func (fexec *FlowExecutor) getDagIntermediateData(context *Context) ([]byte, error) {
	var data []byte
	...
	default:
		...
		if aggregator != nil {
			...
			if serr != nil {
				serr := errors.Newf("failed to aggregate data, error %v", serr)
				return data, serr
			}
			data = sdata
		}
...
func (fexec *FlowExecutor) createContext() *Context {
	context := CreateContext(fexec.id, "",
...
func (fexec *FlowExecutor) init() ([]byte, error) {
	...
	case false: // new request
		...
		fexec.flow = graph.CreatePipeline()
		fexec.id = requestId
		fexec.query = rawRequest.Query
		fexec.dataStore = systemstore.NewInMemoryDataStore()
		...
		if fexec.executor.LoggingEnabled() {
			...
			if err != nil {
				return nil, errors.Wrap(err, "failed to initiate logger")
			}
			fexec.logger = fexec.logger.With().Str("request-id", requestId).Str("workflow", fexec.flowName).Logger()
		}
	...
	case true: // partial request
		...
		fexec.dataStore = systemstore.NewInMemoryDataStoreFromExisting(request.getContextStore())
...
func (fexec *FlowExecutor) applyExecutionState(state *ExecutionStateOptions) error {
	...
	default:
		return errors.Newf("invalid execution state")
	}
	return nil
}
...
func (fexec *FlowExecutor) Execute(state ExecutionStateOption) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Newf("[request `%s`] Failed to init flow, %v", fexec.id, err)
	}
	...
	if err != nil {
		return nil, errors.Newf("[request `%s`] Failed to define flow, %v", fexec.id, err)
	}

	// Validate Definition: Validate Pipeline Definition
	err = fexec.flow.Dag.Validate()
	if err != nil {
		return nil, errors.Newf("[request `%s`] Invalid dag, %v", fexec.id, err)
	}
	...
	if fexec.partial && !fexec.isActive() {
		return nil, errors.Newf("[request `%s`] flow is not running", fexec.id)
	}

	// stateStore need to be defined
	if !stateSDefined {
		return nil, errors.Newf("[request `%s`] Failed, stateStore need to be defined", fexec.id)
	}
	...
	if fexec.hasEdge && !fexec.isExecutionFlow && !dataSOverride {
		return nil, errors.Newf("[request `%s`] Failed not an execution flow, DAG data flow need external dataStore", fexec.id)
	}
	...
	if !fexec.partial {
		...
		serr := fexec.setRequestState(StateRunning)
		if serr != nil {
			return nil, errors.Newf("[request `%s`] Failed to mark dag state, error %v", fexec.id, serr)
		}
		...
		fexec.flow.UpdatePipelineExecutionPosition(graph.DepthSame,
	...
	if fexec.partial && !fexec.flow.Dag.IsExecutionFlow() {
		...
		if gerr != nil {
			gerr := errors.Newf("failed to retrive intermediate result, error %v", gerr)
	...
	if fexec.finished {
		...
		context.State = StateSuccess
		if fexec.flow.Finally != nil {
			fexec.log("[request `%s`] calling Finally handler with state: %s\n",
				fexec.id, StateSuccess)
			fexec.flow.Finally(StateSuccess)
		}
...
func (fexec *FlowExecutor) Stop(reqId string) error {
	...
	if err != nil {
		return errors.Newf("[request `%s`] Failed to init stores, %v", fexec.id, err)
	}

	err = fexec.setRequestState(StateFinished)
	if err != nil {
		return errors.Newf("[request `%s`] Failed to mark dag state, error %v", fexec.id, err)
	}
	fexec.stateStore.Cleanup()
...
func (fexec *FlowExecutor) Pause(reqId string) error {
	...
	if !fexec.isRunning() {
		return errors.Newf("[request `%s`] Failed to pause, request is not running", fexec.id)
	}

	err = fexec.setRequestState(StatePaused)
	if err != nil {
		return errors.Newf("[request `%s`] Failed to mark dag state, error %v", fexec.id, err)
	}
...
func (fexec *FlowExecutor) Resume(reqId string) error {
	...
	err = fexec.setRequestState(StateRunning)
	if err != nil {
		return errors.Newf("[request `%s`] Failed to mark dag state, error %v", fexec.id, err)
	}
	...
	if err != nil {
		return errors.Newf("[request `%s`] Failed to retrive partial state, error %v", fexec.id, err)
	}

	for _, ps := range partialStates {
		err = fexec.executor.HandleNextNode(ps)
		if err != nil {
			return errors.Newf("[request `%s`] Failed to forward partial state, error %v", fexec.id, err)
		}
	}
...
// CreateFlowExecutor initiate a FlowExecutor with a provided IExecutor
func CreateFlowExecutor(executor IExecutor, notifyChan chan string) (fexec *FlowExecutor) {
